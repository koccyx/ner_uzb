...
Для token classification используется схема `BILOU`. При инференсе
ограниченный алгоритм Витерби выбирает наиболее вероятную корректную
последовательность тегов: сущность должна быть либо одиночным `U`-токеном,
либо последовательностью `B ... L` одного класса.
